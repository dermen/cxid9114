...
parser.add_argument("--realdatamode", action="store_true")
...
parser.add_argument("--filteredstrong", type=str, required=True, help="filtered combined strong reflections file")
parser.add_argument("--filteredint", type=str, required=True, help="filtered combined integrated reflections file")
...
if args.maskfile is not None:
    import pickle
    # True means bad, False means good

    mask_data = pickle.load(open(args.maskfile, 'r'))
    IS_BAD_PIX = np.array([m.as_numpy_array() for m in mask_data], np.bool)
    IS_BAD_PIX = np.logical_not(IS_BAD_PIX)

GAIN = args.gain
sigma_readout = 3. / GAIN
nmismatch = 0
nstrong_tot = 0
nstills_pred = 0
nbragg_pred = 0
...
El = ExperimentListFactory.from_json_file(args.filteredexpt, check_format=True)  # check format is it slow ??
...
for i_shot in range(Nexper):
    ...
    if args.imgdirname is not None:  # not this was a hack for simulated data files moving on NERSC
        fpath = fpath.split("/kaladin/")[1]
        fpath = os.path.join(args.imgdirname, fpath)
    _fpath = fpath
    if not args.realdatamode:
        ...
        if args.realdatamode:
            flex_data = iset.get_raw_data(0)
            img_data = np.array([d.as_numpy_array() for d in flex_data])
        else:
    ...
    #   <><><><><><><><><><><><><><><><><><><><>
    # choose a device Id for GPU (doesnt matter if not using cuda, will just use CPU instead)
    device_Id = np.random.choice(range(n_gpu))
    # call the simulation helper
    simsAB = sim_utils.sim_colors(
        crystal, DET, BEAM, FF,
        energies,
        FLUX, pids=None, profile=profile, cuda=args.cuda, oversample=1,
    ...
    if not args.realdatamode:
        ...
        refls_strong = Rmaster_strong.select(Rmaster_strong['id'] == i_shot)
    ...
    results = tilt_fit(
        ...
        exper=exper, predicted_refls=refls_predict, sb_pad=args.sbpad, is_BAD_pix=IS_BAD_PIX)
    ...
    _R_shot_strong = Rmaster_strong.select(Rmaster_strong['id'] == i_shot)
    if not args.realdatamode:
        ...
        _R_shot = Rmaster_int.select(Rmaster_int['id'] == i_shot)
    ...
    writer.create_dataset("resolution/shot%d" % n_processed, data=resolution, dtype=np.int, compression="lzf")
